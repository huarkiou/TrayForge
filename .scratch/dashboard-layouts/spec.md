...
- **Grid layout**: an adaptive grid of square compact cards — status dot,
  name, WebUI copy, toggle, edit — with no output preview. Columns adapt to
  window width.
...
7. As a user, I want the grid cards to have no output preview, so that the
   cards stay dense and square.
...
- **Seam 1 — Dashboard widget tests** (existing dashboard screen test file):
  ...
  switches between the two layouts; grid cards render without an output
  preview but with working controls; tapping a grid card opens the detail
